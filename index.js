	...
	*/
	...
	getShortPath:function(nearPath){
		...
		if(nearPath.length==1){
			...
			if(flag){
				for (var i = 0; i < nearPath.length; i++) {
					pathLength.push({'index':i,"value":this.getPathLength(nearPath[i].crossA,nearPath[i].A)
						+this.getPathLength(nearPath[i].crossB,nearPath[i].B)
						+this.getPathLength(nearPath[i].crossA,nearPath[i].crossB)});					
				}
